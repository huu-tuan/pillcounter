...
def preprocess(image, crop=[0, 0, 0, 0], pad=True):
    ...
    if len(image.shape) == 3: 
        image = image[:, :, 2]  # input image is the same for each layer so grabbing one is more efficient than conversion
    ...
    if pad: 
        image = np.pad(image, pad_width=5, mode='constant', constant_values=255)
    return image
...
class Counter():
    ...
    def diffFrame(self, frame):
        assert frame is not None, "frame NONE"
