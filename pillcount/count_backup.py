...
class video():
    ...
    def __init__(self, windowName, myCounter, frame):
        ...
        self.window = sg.Window("DrugSettings", self.layout, location=(100, 100), size=(1280, 720), margins=(0, 0),
                                border_depth=None, no_titlebar=True)
    ...
    def update(self):
        self.frame = self.myCounter.frame
        self.contours = []
        #if not self.myCounter.CountingIdle: #TODO: re add this later
        if len(self.myCounter.thresh) !=0:
            contours, hierarchy = cv2.findContours(self.myCounter.thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def saveRecentFrames(self):
        recentFrames = copy.deepcopy(self.myCounter.recentFrames)
    ...
    def show(self):
        while not self.stopped:
            ...
            displayFrame = np.array(self.frame, copy=True)  # make a copy so that we don't modify the raw frame
            ...
            if event == "BACK" or event == "-HEADER-" and self.myCounter.systemInitialized:
...
def run(args):
    ...
    if args.InputVideo == '':
        ...
        # """ add """
        # cap2 = cv2.VideoCapture(args.InputVideo2)
        # cap2.set(cv2.CAP_PROP_POS_FRAMES, args.StartFrame)
        # for i in range(10):
        #     time.sleep(0.1)
        #     ret, frameStatic2 = cap2.read()
        # if type(frameStatic2) == type(None):
        #     print('Video File not Found')
        #     quit()
        # frameStatic2 = prep(frameStatic2, crop)
    ...
    myCounter = counter(args.ModbusCom, [1, 2], args.Crop, args.AreaThresh, args.DiffThresh, frameStatic, frameStatic,
                        args.Debug)

    # """ add """
    # myCounter2 = counter(args.ModbusCom, [1, 2], args.Crop, args.AreaThresh, args.DiffThresh, frameStatic2, frameStatic2,
    #                     args.Debug)
    ...
    if os.path.exists(imageFileName):
        ...
        try:
            myCounter.tubeCleanImage = cv2.imread(imageFileName)[:, :, 2]

            # """ add """
            # myCounter2.tubeCleanImage = cv2.imread(imageFileName)[:, :, 2]

        except:
            print('Tube Image likely corrupt')
            os.remove(imageFileName)
            myCounter.tubeCleanImage = frameStatic
            cv2.imwrite(imageFileName, myCounter.frameStatic)

            # """ add """
            # myCounter2.tubeCleanImage = frameStatic2
            # cv2.imwrite(imageFileName, myCounter2.frameStatic)
    ...
    if args.ModbusCom:
        ...
        comStream = communicationStream(myCounter,version=version)
        comStream.start()
        
        # """ add """
        # comStream2 = communicationStream(myCounter2,version=version)
        # comStream2.start()

    # Start GUI Thread
    if args.DisplayVideo:
        myVideo = video('Video', myCounter, frameStatic)
        myVideo.start()
        
        # """ add """
        # myVideo2 = video('Video', myCounter2, frameStatic)
        # myVideo2.start()

    # tr = tracker.SummaryTracker()
    if args.InputVideo == '': 
        myCameraStream.reset()  # reset frame buffer - this will sometimes max out on init only

        # """ add """
        # myCameraStream2.reset()  # reset frame buffer - this will sometimes max out on init only
    ...
    while True:
        ...
        if args.InputVideo == '':
            frame, frameBufferLen = myCameraStream.get()
            
            # """ add """
            # frame2, frameBufferLen2 = myCameraStream2.get()

            if args.DisplayVideo:
                myCounter.frameBufferLen = frameBufferLen
                ...
                # """ add """
                # myCounter2.frameBufferLen = frameBufferLen
                # myVideo2.frameBufferLen = frameBufferLen
                # if myVideo2.frameBufferLen > myVideo2.maxframeBuffer: myVideo2.maxframeBuffer = myVideo2.frameBufferLen
                # if args.plotResults:frameBufferAll.append(frameBufferLen)
        else:
            _, frame = cap.read()
            if args.VideoLoop and frame is None:
            ...
            # """ add """
            # _, frame2 = cap2.read()
            # if args.VideoLoop and frame2 is None:
            #     print('restarting video')
            #     cap2 = cv2.VideoCapture(args.InputVideo2)
            #     _, frame2 = cap2.read()

        # add frame to recent Frame buffer
        myCounter.recentFrames.append(frame)
        if len(myCounter.recentFrames) > 1000:
            myCounter.recentFrames.pop(0)

        # add frame to all frame buffer
        if args.SaveFrames: allFrames.append(frame)

        # prep frame
        if frame is not None:
            frame = prep(frame, crop)
        else:
            break

        # stop if target pill count reached
        if args.NumPillsToCount != 0 and len(myCounter.pillDataFull) >= args.NumPillsToCount: break

        ## Process frame

        # diff frame
        myCounter.diffFrame(frame)

        if args.ModbusCom:
            if comStream.comm_FrontDoorClosedValue:

                # Find Contours
                myCounter.findContours()

                # Count pills based on contours
                myCounter.countPills(frameTotal)
                
            # """ add """
            # if comStream2.comm_FrontDoorClosedValue:

            #     # Find Contours
            #     myCounter2.findContours()

            #     # Count pills based on contours
            #     myCounter2.countPills(frameTotal)
        else:
            # Find Contours
            myCounter.findContours()
        ...
        # write to log file
        if myCounter.pillCount > previousPillCount and args.Logging:
            if myCounter.pillDataFull == 10:
                for pill in myCounter.pillDataFull[0:8]: #this may cause an issue when pillDataFull is reset at 1000
                    pill.log(logFileName,myCounter.puckID)
            if myCounter.pillCount >= 20 and myCounter.pillCount % 10 == 0:
                #Modified to write previous 10 pills
                for pill in myCounter.pillDataFull[len(myCounter.pillDataFull)-11:len(myCounter.pillDataFull) - 1]:
                    pill.log(logFileName,myCounter.puckID)
            previousPillCount=myCounter.pillCount
        ...
        if args.DisplayVideo:
            if myVideo.stopped:
                if args.InputVideo == '': myCameraStream.stop()
                break
        ...
        if args.DisplayVideo and args.PauseOnPill > 0 and len(myCounter.pillDataCurrent) > 0:
            time.sleep(args.PauseOnPill)

        # Clean up
        if len(myCounter.pillDataFull)>1000 and myCounter.CountingIdle: #changed to 1000 to prevent excessive memory use TODO: determine if this causes issues with logging or counting
            myCounter.reset()
    ...
    if args.Debug:
        ...
        for pill in myCounter.pillDataFull:
            print(pill)
        print()
    ...
    return len([x for x in myCounter.pillDataFull if x.Registered])
